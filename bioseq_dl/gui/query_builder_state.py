"""Pure state conversion helpers for GUI query builders."""
...
UNIPROT_BUILDER_FIELD_LABEL_TO_VALUE = {
    f"{entry.label} ({entry.key})": entry.key
    for entry in UNIPROT_QUERY_FIELD_CATALOG.values()
}
...
QUERY_BUILDER_LABEL_TO_KEY = {
    label: key for key, label in QUERY_BUILDER_KEY_TO_LABEL.items()
}
...
def build_uniprot_builder_form_rows(
    ui_rows: list[dict[str, object]],
) -> list[dict[str, object]]:
    """Convert visible GUI builder rows to pure builder form rows."""
    form_rows = []
...
def get_active_chembl_builder_label(builder_label_or_key: object) -> str:
    """Return a ChEMBL builder label, falling back to target when needed."""
...
def get_chembl_field_catalog_for_builder(
    ...
    return get_chembl_query_builder_field_catalog(
        get_chembl_builder_resource(builder_label_or_key)
    )


def get_chembl_builder_field_label(
    builder_label_or_key: object,
    field: object,
) -> str:
...
def get_chembl_builder_field_value(
    builder_label_or_key: object,
    label_or_value: object,
) -> str:
    """Return an internal ChEMBL field key for a visible field label or key."""
    catalog = get_chembl_field_catalog_for_builder(builder_label_or_key)
    text = str(label_or_value)
...
def get_chembl_field_entry(
    ...
    """Return ChEMBL catalog metadata for a visible field label or key."""
    catalog = get_chembl_field_catalog_for_builder(builder_label_or_key)
...
def get_chembl_filter_type_options(
    builder_label_or_key: object,
    label_or_value: object,
) -> list[str]:
...
def build_chembl_builder_form_rows(
    ...
    return [
        {
            "field": get_chembl_builder_field_value(
...
def build_gui_query_builder_state_from_loaded_form(
