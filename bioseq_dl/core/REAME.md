...
- [ ] In uniprot.py, line 413, change `self.parse(response.json(), extract_fields)` to `self.parse_results(response, extract_fields)` because sometimes the response is not json serializable. To handle both cases we can make sure that the response is a json object before calling `response.json()`.
