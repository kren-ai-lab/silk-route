...
def _email_option() -> Any:
    return typer.Option(
        None, "--email", "-e", envvar="BRENDA_EMAIL", help="BRENDA account email (or set BRENDA_EMAIL)."
    )


def _password_option() -> Any:
    return typer.Option(
        None,
        "--password",
        envvar="BRENDA_PASSWORD",
        help="BRENDA password. Prefer the BRENDA_PASSWORD env var over passing it on the command line.",
    )


def _ec_argument() -> Any:
    return typer.Argument(..., help="EC number of the enzyme.")


def _organism_option() -> Any:
    return typer.Option(None, "--organism", "-org", help="Organism name to filter results.")
