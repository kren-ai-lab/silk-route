import typer
from typing import Optional

from bioseq_dl import PrideInterface

app = typer.Typer(help="Collect data from PRIDE database.")

@app.command("search_projects")
def run_search_projects(
    keyword: str = typer.Argument(
        ..., 
        help="Keyword to search for."
    ),
    filter: Optional[str] = typer.Option(
        None, "--filter",
        help="Filter for the search."
    ),
    page: int = typer.Option(
        0, "--page",
        help="Page number for pagination."
    ),
    dateGap: Optional[str] = typer.Option(
        None, "--dateGap",
        help="Date gap filter."
    ),
    sortDirection: str = typer.Option(
        "DESC", "--sortDirection",
        help="Sort direction (ASC or DESC)."
    ),
    sortFields: str = typer.Option(
        "submissionDate", "--sortFields",
        help="Field to sort by."
    ),
    output_file: Optional[str] = typer.Option(
        None, "--output_file",
        help="File to save the results."
    )
):
    """
    Search for projects in the PRIDE database.
    """
    interface = PrideInterface()
    query = {
        "keyword": keyword,
        "filter": filter,
        "page": page,
        "dateGap": dateGap,
        "sortDirection": sortDirection,
        "sortFields": sortFields
    }
    results = interface.fetch_single(
        method="search",
        option="projects",
        query=query,
        parse=True,
        to_dataframe=True
    )

    if output_file:
        results.to_csv(output_file, index=False)
        typer.echo(f"Results saved to {output_file}")
    else:
        typer.echo(results)


@app.command("projects")
def run_projects(
    projectAccession: str = typer.Argument(
        ..., 
        help="Project accession ID."
    ),
    output_file: Optional[str] = typer.Option(
        None, "--output_file",
        help="File to save the results."
    )
):
    """
    Get details of a specific project by its accession ID.
    """
    interface = PrideInterface()
    query = {
        "projectAccession": projectAccession
    }
    results = interface.fetch_single(
        method="projects",
        option="default",
        query=query,
        parse=True,
        to_dataframe=True
    )

    if output_file:
        results.to_csv(output_file, index=False)
        typer.echo(f"Results saved to {output_file}")
    else:
        typer.echo(results)

@app.command("similar_projects")
def run_similar_projects(
    accession: str = typer.Argument(
        ..., 
        help="Project accession ID to find similar projects."
    ),
    page: int = typer.Option(
        0, "--page",
        help="Page number for pagination."
    ),
    pageSize: int = typer.Option(
        10, "--pageSize",
        help="Number of results per page."
    ),
    output_file: Optional[str] = typer.Option(
        None, "--output_file",
        help="File to save the results."
    )
):
    """
    Get similar projects to a given project accession ID.
    """
    interface = PrideInterface()
    query = {
        "accession": accession,
        "page": page,
        "pageSize": pageSize
    }
    results = interface.fetch_single(
        method="projects",
        option="similarProjects",
        query=query,
        parse=True,
        to_dataframe=True
    )

    if output_file:
        results.to_csv(output_file, index=False)
        typer.echo(f"Results saved to {output_file}")
    else:
        typer.echo(results)