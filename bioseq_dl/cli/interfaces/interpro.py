...
def run_entry(
    id: str = typer.Option(
        None, "--id", "-id",
        help="InterPro entry ID to fetch."
    ),
    db: str = typer.Option(
        None, "--db", "-db",
        help="Specific database within InterPro to query (e.g., InterPro)."
    ),
    filter_uniprot_accession: str = typer.Option(
        None, "--filter-uniprot-accession", "-fua",
        help="Filter results by a specific UniProt accession."
    ),
    filter_taxonomy: str = typer.Option(
        None, "--filter-taxonomy", "-ft",
        help="Filter results by a specific taxonomy ID."
    ),
    ...
    if filter_uniprot_accession:
        query["filters"] = [{
            "type": "protein",
            "db": "reviewed",
            "value": filter_uniprot_accession
