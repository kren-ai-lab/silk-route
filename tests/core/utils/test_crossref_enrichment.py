...
def test_run_skips_when_no_fields():
    data, meta = run_crossref_enrichment(pl.DataFrame({"a": [1]}), crossref_fields=[])
    assert data == {}
    assert meta == {"skipped": True, "reason": "no_crossref_fields"}


def test_run_skips_when_input_empty():
    data, meta = run_crossref_enrichment(pl.DataFrame(), crossref_fields=["kegg"])
    assert data == {}
...
def test_run_skips_when_no_endpoint_specs_resolved():
    ...
    data, meta = run_crossref_enrichment(pl.DataFrame({"a": [1]}), crossref_fields=["definitelynotadatabase"])
    assert data == {}
