...
def _resolve_log_dir() -> Path:
    ...
    Precedence: (1) the ``SILKROUTE_LOG_DIR`` env var if set, (2) the
    ``silkroute.core.config`` cache log path if available, (3) ``~/.cache/silkroute/logs``.

    Returns:
        Path: The resolved log directory.

    """
    env_dir = os.environ.get("SILKROUTE_LOG_DIR")
    if env_dir:
        return Path(env_dir).expanduser().resolve()

    # Optional: use project config if available (avoids a hard dependency)
    with contextlib.suppress(Exception):
        from silkroute.core import config  # ty: ignore  # type: ignore[import]  # noqa: PLC0415

        cfg = config.get_config()
        return Path(cfg.cache_paths.logs()).expanduser().resolve()

    return Path("~/.cache/silkroute/logs").expanduser().resolve()
...
class _LoggingManager:
    ...
    def configure(
        ...
        *,
        ...
        ``SILKROUTE_LOG_DIR`` is honored (useful for pytest fixtures). The same env
        ...
        # so that env var SILKROUTE_LOG_DIR set at runtime takes effect.
        ...
        env_flag = os.environ.get("SILKROUTE_LOGGING", "").strip().lower()
    ...
    def _build_handlers(self) -> list[logging.Handler]:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                sys.stderr.write(
                    f"[silkroute.logging] WARNING: Failed to initialize file handler at "
...
def configure_logging(
    *,
    ...
    """Configure silkroute logging once at program start (optional).
