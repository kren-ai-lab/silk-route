# silkroute/core/utils/query_builders.py
"""Query builders for cross-reference queries.

These functions take a row of a DataFrame (a ``{column: value}`` dict), check
whether the fields required for the query are present, and return a list of query
parameters for the corresponding API call.
"""
...
QueryBuilder = Callable[[Mapping[str, Any], dict], list]


def _is_missing(value: Any) -> bool:
    """Return whether a row value is missing (None or a float NaN)."""
...
def to_str_list(value: Any) -> list[str]:
    ...
    Handles: missing values, scalar strings, JSON-like strings ('[a,b]'),
    lists/tuples, and returns a list of non-empty strings.
    """
    # Missing
...
def build_query_alphafold_prediction(row: Mapping[str, Any], _params: dict) -> list:
    """Build AlphaFold prediction queries from 'alphafold_ids' column."""
    return to_str_list(row.get("alphafold_ids"))


@register_query_builder("biodbnet", "db2db")
def build_query_biodbnet_db2db(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_biodbnet_getpathways(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_biogrid_interactions(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_brenda(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_chembl(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_chebi_compounds(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_chebi_ontology(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_kegg(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_panther_familymsa(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_panther_geneinfo(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_pathwaycommons_fetch(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_pathwaycommons_top_pathways(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_pathwaycommons_neighborhood(row: Mapping[str, Any], params: dict) -> list:
    """Build PathwayCommons neighborhood queries from 'accession' and 'organism_id' columns."""
    accession = None if _is_missing(row.get("accession")) else row.get("accession")
    organism = to_str_list(row.get("organism_id"))
...
def build_query_pdb(row: Mapping[str, Any], _params: dict) -> list:
...
def build_query_pubchem_compound_summary(row: Mapping[str, Any], params: dict) -> list:
    """Build PubChem compound summary queries from 'gene_primary' and 'organism_id' columns."""
    if not _is_missing(row.get("gene_primary")) and not _is_missing(row.get("organism_id")):
        gene_primary = (
...
def build_query_pubchem_protein(row: Mapping[str, Any], params: dict) -> list:
    """Build PubChem protein queries from 'accession' column."""
    if not _is_missing(row.get("accession")):
        return [{"accession": row["accession"], **params}]
    return []


@register_query_builder("reactome", "data-discover")
def build_query_reactome(row: Mapping[str, Any], _params: dict) -> list:
...
def build_query_rhea(row: Mapping[str, Any], params: dict) -> list:
...
def build_query_refseq(row: Mapping[str, Any], _params: dict) -> list:
...
def build_query_sabiork_kineticlaws(row: Mapping[str, Any], params: dict) -> list:
