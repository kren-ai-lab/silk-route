...
from silkroute.core.cache import clear_cache, list_caches
from silkroute.logging import get_logger

app = typer.Typer(name="cache", help="Cache utility commands for SilkRoute.")


@app.command("list")
def list_registered() -> None:
    """List registered caches."""
    log = get_logger("silkroute.cli.cache.list")
    regs = list_caches()
    if not regs:
...
def clear(
    ...
    """
    log = get_logger("silkroute.cli.cache.clear")
